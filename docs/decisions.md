...
## [2026-08-01] 在本机（RTX 5080）重立全部数值基线，两侧一起重跑

**背景**：2026-08-01 从 RTX 5070 Ti 迁到本机后，约定 6 的锚点哈希失效、`docs/experiments.md` 的数字不再逐位重现（`investigations.md#cross-machine-hash`）。同日又测出**参照侧同样绑定机器**——MMSA 自己的代码跑 tfn seed 42，旧机 MAE 0.9305、本机 0.9441。此前一直挂着「要不要重立基线」未决。

**选项**：A 维持现状，承认已入库数字属于旧机器，只对本机新结果另立一套 / B 只重跑我们这一侧 / C **两侧一起重跑**：我们的 38 个实验组 + MMSA 代码参照的全部模型

**裁定**：**C**，由用户拍板。范围是"当前机器再跑一遍所有模型和数据"：`reproduce_all.sh` 的全部 38 组，以及 `mmsa_reference.py` 的全部参照模型；锚点哈希在本机重新生成并标注绑定机器。

**依据**：判据比较的是"我们"与"参照"两个分布，**其效力来自配对两侧同机器，而不是来自数字本身的绝对值**。B 会把两台机器混进同一张聚合表——那比两侧都不重跑更糟，因为差异会被读成实现差异。A 是安全的，但代价是此后每加一个模型都要同时维护两套机器的数字，且旧机器已不可用、任何疑问都无法回头验证。

**已否决**：B —— 见上，单侧重跑制造的是不可归因的混合。A —— 旧数字的价值在于可复查，而产生它们的机器已经没了；把不可复查的数字当作基线继续用下去，等于把"复现"建立在一个无法复现的锚点上。
...
## 2026-08-04 — 研究方向：把对比学习作为一类做深（导师指定）

**背景**：用户的导师要求研究向**对比学习**方向推进。**此条来源是导师指定，不是本仓库数据推导** ——`roadmap.md` 里其余方向依据全部标注为自测数据，故这一条必须单独标明来源，避免日后被当成数据结论引用。

**选项**：
- A. 以对比学习为核心**提出新方法**投顶会
- B. **把对比学习作为一类做深**：系统复现并严格评测该类方法
- C. 仅作为选题倾向记录，当前计划不变

**裁定：B**（用户确认）。

**依据**：本项目已建成的三件仪器——10 seed、作者代码同协议参照、权重复制等价测试——恰好是评测这一类所缺的东西。且第一个样本已经给出信号：**ConFEDE 论文主打的「标签距离加权对比损失」在其自己的代码里从未被启用**（`update_label` 定义了三份，无一处调用），这类「论文声称 vs 代码实际」的差异只有权重复制等价测试这一档力度才查得出来。

**范围**：已枚举的 101 篇在范围内论文中，明确属于该类的有 6 篇（CLMLF/NAACL-F 2022、Uni-Modal Coding+Cross-Modal Prediction/EMNLP-F 2022、**ConFEDE**/ACL 2023、ConKI/ACL-F 2023、Supervised Angular Margin CL/EMNLP-F 2023、CLGSI/NAACL-F 2024），边界两篇建议并入：**MMIM**（EMNLP 2021，互信息下界+CPC，**已复现完成**）与 FINE（AAAI 2026，互信息估计）。**共 8 篇，已完成 2 篇。**

**已否决**：A——在没有把这一类的现状测清楚之前提新方法，等于把结论建在未经检验的基线上；这与本项目全部方法论相悖。

**复查条件**：若「论文声称 vs 代码实际」的核对在 8 篇里只发现一两处孤例，则该切入点不足以支撑一篇评测论文，届时应重新评估是否转向 A。

**不改变的部分**：验收标准（10 seed、作者代码参照、等价测试）一律照旧。选题倾向不是放宽标准的理由。
