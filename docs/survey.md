...
固化为 **`scripts/sweep_venues.py`**（`--offline` 用缓存重跑）。结果入库为 [`sweep_2024_2026.tsv`](sweep_2024_2026.tsv)，含全部命中及分档，可复查。

| 出处 | 枚举总数 | 命中 | 同任务 |
|---|---|---|---|
| ACL 系（12 卷，2024–2026） | 15,834 | 94 | 13 |
| CVF（CVPR/ICCV/WACV，2023–2026） | 17,118 | 19 | 10 |
| AAAI（AAAI-24/25/26，97 个分册） | 11,271 | 53 | 17 |
| **合计** | **44,223** | **166** | **40** |

**关键词搜索在这三家只找到 11 篇，逐卷枚举找到 40 篇——漏掉约四分之三，且毫无征兆。** 漏得最狠的两处：**CVPR 2026 有 7 篇**（关键词只找到 EBMC 一篇），**AAAI 三届共 17 篇（关键词一篇都没找到）**。
...
分档规则：`IN_SCOPE` = 多模态情感（MOSI/MOSEI 系）；`adjacent` = 命中窄词但属 ABSA / 对话情绪识别；`other_task` = 只命中宽词。**`IN_SCOPE` 的含义是"值得核"，不是"已确认同任务"**——任务归属只有正文的数据集能定（见下）。
