...
### 第五类阻塞：release 的显存需求超过本机（2026-08-06）

FeaDA 的作者代码参照**跑不起来**：它在自己的配置下峰值超过 15.2 GiB，而本机 GPU 是 15.47 GiB 全部可用量。
原因是伙伴采样——批 32 个锚样本各带 6 个伙伴，224 条 500 帧序列同时过编码器。

试过 `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`（只改分配器、不碰任何数值），仍然不够。

**不降批大小**：批大小是协议的一部分，为了跑得动而改它，得到的就不是这篇论文的参照了。
（同 DPDF-LQ 那次的教训：猜协议值 2.7 SE。）

**故 FeaDA 无作者代码参照，按本项目标准不予判定**——与 ConFEDE 在参照到位前的处理一致。
它的实现已通过等价测试（196 张量、前向 7.451e-09、**对比损失 0.000e+00**），10 seed 也已跑完，
但**这些数字进表时必须标注「无参照」**。

**这是第五类阻塞**，前四类是「无作者代码」「任务不同」「特征口径不同」「必需产物不可得」。
与第四类一样，**这是我们硬件的属性，不是论文的属性**——换一块更大显存的卡就能解开。

### 外部综述表的核对与并入（2026-08-06）

用户提供了一份 GPT 整理的综述表（17 行，2017-2026）。按既定用法——**结构当范本，条目走核实**——
逐项对过我们已有的实测与出处：
...
| 模型 | 我们 | 表格 |
|---|---|---|
| MMIM | **EMNLP 2021** | CVPR ❌ |
| DMD | **CVPR 2023 highlight** | EMNLP ❌ |
| DLF | **AAAI 2025** | 待确认 |
| DPDF-LQ | **EMNLP 2025** | 待确认 |

**Acc-7 一列混了三种来源，因此不可用**：

| 模型 | 我们 10 seed 实测 | 表格 | 差 |
|---|---|---|---|
| **MulT** | 36.55 ± 1.66 | 45.30 | **+8.75** |
| MISA | 42.08 ± 0.95 | 46.10 | +4.02 |
| CLGSI | 44.53 ± 2.22 | 47.96 | +3.43 |
| DMD / DLF / DPDF-LQ | 45.31 / 44.85 / 44.58 | **逐位相同（含标准差）** | 0 |

后三行是**我们自己的数字被抄了进去**（连 ±1.57 都一致），其余是论文自报值。
**同一列并置「论文自报」与「10 seed 验证集选轮」，这一列就失去意义**——MulT 差 8.75 个百分点，
正是本项目要区分的那种差距。

**并入的部分**：只并「创新类型」（融合/对齐/表示学习/对比学习/蒸馏），这是我们没有的维度，
已写进 `papers.tsv` 的 `class` 列，15 条。**数字与出处一概不并入。**

**并入时踩的一个坑**：第一版按 `title.startswith(模型名)` 匹配，`mult` 匹中了每一个
「Multimodal …」开头的标题，一次给 6 篇错贴「对齐」。**名字短的时候，前缀匹配等于没有匹配**——
改为 key 精确匹配加词边界后为 15 条，逐条可核。

### 覆盖度差集：期刊是真缺口

表格有而我们 129 篇里没有的 3 篇：**HyCon（TMM 2022）**、MMCL（arXiv，即已核 EMNLP-F 2022 那篇的预印本）、
MAG-BERT（我们有，key 是 `bert_mag`，仅名字未对上）。

**HyCon 暴露了真实缺口：`sweep_venues.py` 只枚举会议，不扫期刊。** TMM / TAFFC / TASLP 等
期刊上有同任务工作，目前一篇都没进过枚举。已列入待办。

我们有而表格没有的 116 篇——这个方向的差距说明该表覆盖面远小于逐卷枚举，符合预期。
...
## 核实记录：对比学习一类（2026-08-05）

按导师指定方向（`decisions.md` 2026-08-04）逐篇核实，**一律读正文，不看会议页面元数据**。

| 方法 | 出处 | 用 CMU-MOSI | 作者代码 | 结论 |
|---|---|---|---|---|
| **CLGSI** | Findings of NAACL 2024, pp.2099-2110 | ✅ MOSI/MOSEI/SIMS | [AZYoung233/CLGSI](https://github.com/AZYoung233/CLGSI)，**MIT** | **下一个做**——有作者代码，可做等价测试 |
| ConKI | Findings of ACL 2023 | ✅ MOSI/MOSEI/SIMS | **无**（正文唯一的 GitHub 链接是它所用的框架 `thuiar/MMSA`，非自有代码；作者仓库亦未见） | 只能自实现，做不了等价测试，优先级下调 |

**ConKI 这条是「核实规矩」的又一个例证**：ACL Anthology 页面既不显示数据集也不显示代码链接，
必须下载正文才能判断。而正文里出现 GitHub 链接**不等于**有作者代码——它可能只是所用框架的链接。

**CLGSI 值得优先的另一个理由**：它的核心机制是「按情感强度差异挑选正负样本对并据此加权」，
