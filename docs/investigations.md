...
| TFN 在 MOSEI 上的配置 | valid MAE |
|---|---|
| MOSI 超参 + 我们的默认池化（材料原用） | **0.5334 ± 0.0052** |
| **MMSA 的 MOSEI 超参** + 我们的默认池化（修正） | 0.5387 ± 0.0040 |
| 参照：MMIM(off) + `label_dcl`（全套损失工程） | 0.5333 ± 0.0083 |

**MMSA 那套「正确的」MOSEI 超参在我们的实现上反而更差。** 合理——它是针对 MMSA 自己那版（padded-width 池化、读 PAD 之后的 LSTM 状态）调的，换到我们的改进版就不最优。这本身是一条小发现：**跨实现变体的超参不可直接搬用。**

因此材料里的对照**成立且更强**：两套都没有为「我们的实现 + MOSEI」调过的 TFN 配置，都落在 0.533–0.539，而 MMIM 加上 14 候选筛选 + 组合 + 自适应权重 + 新损失设计落在 0.5333。TFN 的 seed 方差还小近一半（0.0040-0.0052 对 0.0083）。
...
> **2026-09-10 当日更正（两次）。** 这里原本写"它抬高的正是论文要报的那些数字"、
> "方向必然是让论文数字更好"——**凭机制推断的，方向是反的**。跑到两个 seed 时看到
> clean 臂明显更好（MAE 0.7020 对 0.7127、0.7431 对 0.7484），我据此写了"clean 更好"
> ——**这也是抢跑**。五个 seed 跑完的结论是**测不出来**，见下面那节。重新推导机制：
>
> - **clean** 上报的是**验证损失最低**的那一轮（最后一次验证改善）。
> - **as-released** 要求验证改善**且**测试改善，接受集是 clean 接受集的**子集**，
>   所以它上报的轮次**不晚于** clean 的。
>
> 那个合取条件让它**提早冻结**：测试损失一停止改善就不再更新，于是上报一个训练不足
> 的轮次。门槛作用在测试 **MSE** 上、确实压低了所选轮次的测试 MSE（实测配对差
> −0.04），但论文报的是 MAE / Corr / Acc-7，**早冻结的代价大于偷看的收益**。
>
> 所以机制上能确定的只有两件事：**泄漏是真的**（测试集确实被碰了、被用来选轮次），
> 以及**上报轮次不晚于 clean**（结构性后果，接受集是子集）。至于它对 MAE / Corr /
> Acc 这些上报指标是抬高还是压低，**测不出来**。逐轮数字见
> `scripts/hscl_selection_trace.py`。
>
> 这条记下来的价值不在结论，而在于**我连着抢跑了两次**：先凭机制断言方向（反了），
> 再凭两个 seed 断言方向（没撑到第五个）。两次都是同一个毛病——**在预先定好的样本量
> 跑完之前读方向**。本仓库为此栽过三次（`#contrastive-screen-noise` 那三条假线索），
> 判据早就写好了，是我自己没守。

### 五个 seed 的结论：测不出来

seeds 42-46，非配对 Welch（配对的前提不成立，见 `#hscl-nondeterministic`）：

| 指标 | as-released | clean | 差 | p 双侧 | MDE |
|---|---|---|---|---|---|
| MAE ↓ | 0.7371 ± 0.0183 | 0.7315 ± 0.0205 | +0.0057 | 0.656 | **0.0338** |
| Corr ↑ | 0.7811 ± 0.0104 | 0.7895 ± 0.0089 | −0.0084 | 0.208 | 0.0169 |
| Acc-7 ↑ | 0.4574 ± 0.0148 | 0.4525 ± 0.0113 | +0.0050 | 0.569 | 0.0229 |
| Acc-5 ↑ | 0.5160 ± 0.0149 | 0.5146 ± 0.0164 | +0.0015 | 0.887 | 0.0272 |
| Acc-2 non0 ↑ | 0.8378 ± 0.0108 | 0.8384 ± 0.0115 | −0.0006 | 0.934 | 0.0194 |
| Acc-2 has0 ↑ | 0.8192 ± 0.0090 | 0.8222 ± 0.0088 | −0.0029 | 0.618 | 0.0155 |

**六个指标的观测效应全部小于各自的 MDE**，方向不可读（四个偏 clean、两个偏发布版，
p 全在 0.2 以上）。

**要多少 seed 才够？** 把 MAE 的 0.0057 做到 80% 检出力，每臂需要 **n≈143**，两臂
286 次运行、约 **12 小时**。不值得花，**而这个数字本身就是结论**：

> **HSCL 那条选择规则的效应，小于这个模型自身的运行噪声在任何合理预算下能分辨的量。**

它的 seed 标准差是 0.018-0.021 MAE，**比我们 MMIM 的 0.012 大近一倍**，而这个领域
论文声称的改进普遍在 0.01-0.03。所以这条测不出来，与"泄漏无害"是两件不同的事——
**是这台仪器读不出这个刻度**。
...
## HSCL 在固定 seed 下也不可复现 {#hscl-nondeterministic}

**2026-09-10。** 查两臂差异时撞见的，比两臂的结论本身更要紧。

两臂的 per-epoch 轨迹**从第 1 轮就不同**：

| seed 42，第 1 轮 | valid loss | test loss |
|---|---|---|
| as-released | 1.1794 | 1.1070 |
| clean | 1.1824 | 1.1079 |

**第 1 轮那行打印在任何 save 分支之前**，所以不可能是补丁造成的（补丁只动选择）。
seed 43 也一样，第 2 轮起就分岔。

原因：HSCL 的 `set_seed()` 设了 `torch.backends.cudnn.deterministic = True`、
`benchmark = False`，但**那只管 cuDNN 卷积**。BERT 与 `TransformerEncoder` 反向里的
原子加、以及若干 index/scatter 归约在 CUDA 上本身不确定，要靠
`torch.use_deterministic_algorithms(True)` 才压得住——它从未设置。

对本仓库的两个直接影响：

1. **`scripts/patches/README.md` 里"两臂轨迹逐轮相同"那句是我没验证就写下的假设，
   而它是错的。** `report_hscl_arms.py` 据此用了配对检验，已改为非配对 Welch。
   代价是两臂之差里混进运行间噪声，报告时必须连这一点一起说。
2. **它给"报告口径宽于测量精度"又添一条**：这篇论文的单 seed 数字连自己重跑都对不上。
   同 seed 重跑的实测见 `outputs/hscl_arms/determinism/`。

对照本仓库的做法：`check_repro.py` 是 12 道闸门之一，专门问"同一台机器连跑两次是否
