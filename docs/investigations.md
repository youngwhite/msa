...
## <a id="rebaseline-5080"></a>在本机重立基线：一个模型的判定翻面，一个整体结论退回打平（2026-08-02）

按 `decisions.md` 2026-08-01 的裁定，两侧一起在 RTX 5080 上重跑：我们的 38 个实验组（5 小时，无一失败），MMSA 代码参照的 16 个模型 × 10 seed（4.5 小时，160 次运行，无一失败）。代码一个字没改，判据一个字没改，**变的只有底下那块 GPU**。

### 结果一：MFN 从「复现成功」变成「未复现」

MAE 0.9437 → 0.9801（+4.4 SE），Corr 0.6624 → 0.6537（+4.7 SE）。**其余 13 个模型判定不变。**

判定不依赖 sd 封顶：按噪声地板封顶算是 4.3 SE，用真实 sd 0.0628 算是 2.7 SE，**两种算法都超过 2 SE**。

**第一个假设被数据否掉了。** "MFN 的验证曲线特别平，所以选择容易翻"——换 epoch 是普遍现象，不是 MFN 独有：

| 组 | 选择余量（中位/最小） | 换了 epoch 的 seed |
|---|---|---|
| mfn | 0.0111 / 0.0002 | 9/10 |
| tfn | 0.0070 / 0.0002 | **10/10**，均值只动 +0.0029 |
| lmf | **0.0018** / 0.0001 | 10/10，均值只动 +0.0064 |
| misa | 0.0040 / 0.0005 | 10/10 |

LMF 的曲线比 MFN 平六倍，位移却只有六分之一。**"选择翻转"是所有模型的共同处境，解释不了 MFN。**

MFN 是 15 个组里唯一的离群值：ΔMAE +0.0364（第二名 text_bert +0.0174），且**唯一一个位移与 sd 同时增大**的（0.0398 → 0.0628）。逐 seed 双向抖动（+0.20 / +0.13 / −0.11），像方差变大而非系统性偏移。

**还有一层机制此前没记过。** seed 51 新旧都选了 epoch 12，test MAE 却从 1.0092 变成 0.8966——**同一个 epoch 序号、不同的权重**。所以是两层叠加：轨迹本身先分岔，early stopping 再放大。`#cross-machine-hash` 当时只记了放大那一半。

### 2×2：元凶是幅度缩放，不是「常量流」

`mfn_mosi` 验收组跑的是 MMSA 的配置（`need_normalized` + `need_model_aligned`：音视频按**填充宽度**求平均成一步，再沿文本时间轴重复）。我们的 `masked_mean(audio, None)` 与其 `data_loader.__normalize()` 的 `np.mean(axis=0)` 逐行一致——**移植无缺陷，行为是继承的**。

但该配置一次改了两件事：流变成常量，且每个样本被乘上 `valid_len/50`。新增 `collapse_over_real_length` 只去掉后者：

| 配置 | MAE | sd | 极差 |
|---|---|---|---|
| MMSA 配置（常量流 + 缩放） | 0.9801 | 0.0628 | 0.2119 |
| **只去掉缩放**（`reallenmean`） | 0.9404 | **0.0281** | 0.0933 |
| 逐步流（`realseq`） | 0.9459 | 0.0198 | 0.0603 |

去掉缩放，方差降 55%（F=4.98, p=0.025）；再换回逐步流只再降一点，**且不显著**（F=2.03, p=0.31）。**幅度缩放是主因。** 机制说得通：按填充宽度平均等于把音视频特征乘上句长比例，MOSI 句长差异大，同一特征的幅度被句长任意缩放。

**限定（不可略）**：新那格**没有旧机器的对照**——老机器已不可用。这里用的是「同机 seed 间方差」当「跨机器脆弱性」的代理量，两者相关但不是一回事。故本节只声称"缩放是方差的主要来源"，**不声称"已证实跨机器不稳定源于缩放"**。

**不得据此更换验收组。** 去掉缩放后对 MMSA 表的差距从 +4.4 SE 缩到 +1.5 SE——这恰恰是不能用的理由：MMSA 跑的就是带缩放的配置，换一个更好的配置去过判据即朝目标调参。此发现的用途是**解释**，不是修复。

### 结果二：聚合判定 AHEAD → LEVEL

| | 旧机 5070 Ti（两侧同机） | 本机 5080（两侧同机） |
|---|---|---|
| MAE | Z = −2.21（p=0.027）**超过** | Z = **+0.17**（p=0.864）打平 |
| Corr | Z = −3.17（p=0.0015）**超过** | Z = **+1.88**（p=0.060）打平 |
| 符号检验 | 3/14 更差 | 5/14 更差（p=0.42） |

**成因可定位到一个模型。** Corr 的逐模型 z：ef_lstm **+14.86**，其余 13 个在 −2.49…+0.43 之间。判据内置的敏感性分析（不参与裁定）：去掉 ef_lstm 后 Corr Z=−2.17（p=0.030）、MAE Z=−1.27，**仍是"超过"**。

而 EF-LSTM 的 z 来自崩溃计数的抽样运气，不是实现差异：

| | 崩溃 | MAE | Corr |
|---|---|---|---|
| 我们（MMSA 配置） | 2/10（seed 48、51） | 1.0665 | 0.5704 |
| MMSA 代码（本机） | **0/10** | 0.9846 | 0.6435 |
| 我们（2021 配置） | 0/10 | 0.9565 | 0.6594 |

崩溃率按 30 seed 测得约 23%（`#ef-lstm-collapse`），两份实现都有。在 p≈0.23 的伯努利下，0/10 出现约 7%、2/10 约 27%——**两个结果毫无冲突**。

### 应当被记住的结论

**同一套代码、同一套判据、同一批数据，换一张同代显卡，一个 p=0.0015 的整体结论退回"打平"，一个模型的复现判定翻面。** 直接成因是一个已知有 23% 崩溃率的配置在两次抽样里崩了 2 次对 0 次。

这比"我们超过 MMSA"重要得多：它用本项目自己的数据证明了路线图动机第 1 条——**用单机、单 seed、无方差的数字排出来的名次，连换一张同代显卡都扛不住。**

正式表述应为：**对能复现的参照至少打平**（MAE p=0.86，Corr p=0.06）；整体结论对单个不稳定配置高度敏感，排除 EF-LSTM 后为"超过"（Corr p=0.030）；**且该结论不跨机器稳定**。

### 判据的盲区，以及为什么本轮不改

z=+14.86 里大部分来自崩溃运行同时拉低均值、撑大 sd，而现行判据只封顶 sd、不识别崩溃。`#ef-lstm-collapse` 早已挂着这条。

**本轮不动判据。** 已经看到"改了会让结论变好"，此刻定义的任何稳健性规则都无法自证不是为迎合结果而设——`roadmap.md` 写明"不得在已有结果之后调整以迎合结果"。处理方式是**预注册**：规则先记进 `decisions.md`，在 MOSEI 或第 2 阶段**首次应用**，届时同时报告新旧两套判据的结果，**不追溯适用于本轮**。

### 约定 6 的锚点哈希，在本机重立

| 路径 | 本机哈希 | 旧机（已失效） |
|---|---|---|
| `lf_lstm --seeds 42 --device cuda` | `199f629bfb036c93` | `172967c7dced83b2` |
| `lf_lstm --seeds 42 --device cpu` | `969d5a4579736f8f` | `0201e2133a68f4e9` |

**两个都绑定 RTX 5080 + AMD Ryzen 7 7700 这台机器**，换机器后必然失效，且无法靠任何代码改动恢复。

顺带修掉一个漏洞：`check_reproduction.py` 把 CPU 路径的变化报成"**同一硬件上结果变了**"，因为它比对的 `device_description` 对 CPU 只写 `cpu (8 threads)`，不含 CPU 型号——两台机器看起来一样。`env` 现在记 `cpu` 与 `cpu_capability`（2026-08-01 起），该比对应改用它们。

---

## <a id="mctn-batch-axis"></a>MCTN 的 Corr 缺口：MMSA 把批次维当成了时间维（2026-08-02）

我们的 MCTN 对 MMSA 代码参照的 Corr 差 **+8.3 SE**（0.6250 对 0.6661），而 MAE 只差 +1.6 SE。"校准接近、排序明显更差"这个模式触发了约定 5 的排查。**结论是参照实现的数值无效，不是我们的移植有缺陷。**

### 现象

`MMSA/models/singleTask/MCTN.py` 的 `Encoder` 用 `nn.LSTM(...)`（`batch_first` 未设，默认 `False`），而 `MCTN.forward` 喂进去的是 `(batch, seq, feat)`：

```python
enc = Encoder(input_dim=768, hid_dim=32, dropout=0, depth=1, bidirectional=True)
out, (h, c) = enc.rnn(torch.randn(4, 50, 768))
# out.shape == (4, 50, 64)   ← 时间轴被当成了 batch
# h.shape   == (2, 50, 32)   ← 隐状态的 "batch" 是 50，即真实的时间步数
```

**递归沿批次维展开**：50 个时间步被当作 50 个独立样本，而一个 batch 里的 4 个样本被当作 4 个时间步。同一文件里 `Seq2Seq.forward` 的两个轴也是反的——`batch_size = src.shape[1]`（取到序列长度）、`trg_len = trg.shape[0]`（取到批大小），解码循环 `for t in range(1, trg_len)` 因此在批次维上迭代。

**解码器反而是对的**：`dec_input.unsqueeze(1).transpose(0,1)` 给出 `(1, batch, feat)`，符合 `batch_first=False`。同一个文件里编码器与解码器的轴向约定不一致。

形状恰好能对上（`(batch, seq, feat)` 与 `(trg_len, batch_size, dim)` 在这里同形），所以不报错。

### 决定性检验：批次泄漏

正确的模型里，一个样本的预测不依赖于同批次里有哪些别的样本。

| | 预测是否随批次组成变化 | max\|Δ\| |
|---|---|---|
| **MMSA 的 MCTN** | **是** | 6.98e-03（随机输入）/ **3.05e-02**（真实 MOSI 测试样本，仅改 batch size 32→16） |
| 我们的 MCTN | 否 | 1.04e-07（浮点噪声） |

真实数据上，同一批权重、同一批样本，**只把 batch size 从 32 改成 16，逐样本预测就变**，最大差 3.05e-02——标签量程约 6，而本领域的 MAE 差距通常在 0.01–0.05 量级。**这不是舍入。**

### 归属：MMSA 引入的，不是从原作者继承的

约定 5 要求发现参照缺陷时必须回原作者确认属于哪一类。原作者实现（[hainow/MCTN](https://github.com/hainow/MCTN)，Keras）在 `seq2seq/mctn_models.py` 里写 `shape = (batch_size,) + (input_length,) + (input_dim,)`，而 **Keras 的循环层按约定就是 batch-first**——作者的轴向是对的。

**所以这是 PyTorch 二次实现引入的缺陷。按约定 5，这一类不该复现。**

### 后果

1. **那 +8.3 SE 不是我们的 bug**，是我们正确、参照错误。参照跑的根本不是时序模型——它的"时间"是批次索引。
2. **MMSA 的 MCTN 数字不能作参照值**：它的测试集预测取决于样本如何分批，换 batch size 或换顺序数字就变。这比 `#mmsa-all-eleven` 里"表不可达"严重一级——那是目标值偏乐观，这是**数值本身无定义**。
3. **错的那版 Corr 反而更高**（0.6661 对 0.6250）。不奇怪：跨样本递归相当于给了模型一条批次级信息通路，且测试时仍然开着。**"参照更好"再一次是缺陷信号而不是好消息**——与 `#almt-better-than-reference` 同一个教训，只是这次方向相反：错的是参照。

### 与 ALMT 那次的对照

ALMT：我们更好 → 查出是**我们**错。MCTN：参照更好 → 查出是**参照**错。两次都靠"不接受任何一方更好作为好消息"这条规矩抓出来。**这条规矩的价值不在于怀疑谁，在于两个方向同等力度地怀疑。**

### 连带暴露的判据缺口：聚合判据不读裁定（2026-08-02）

把 mctn 与 mfm 并入聚合判据后，Corr 从 **Z=+1.88（p=0.060，打平）** 变成 **Z=+3.35（p=8.0e-04，系统性落后）**。翻转几乎全部来自 **mctn 的 z=+8.34**——一个本节刚刚证明"预测依赖批次组成、数值无定义"的参照值。

**逐模型判据有 `acceptance_status.json` 的裁定机制，聚合判据没有。** 于是一个被裁定为参照无效的模型，仍然以满权重拉低整体结论；ef_lstm（z=+14.86，崩溃计数抽样运气）同样如此。两个已裁定项合计主导了整个 Corr 聚合。

**本轮不改判据。** 已经看到"让聚合读裁定会让结论变好"，此刻定义的任何规则都无法自证不是为迎合结果而设——与 `#rebaseline-5080` 末节对 EF-LSTM 稳健性规则的处理相同。

**预注册**（与稳健性规则一并，在 MOSEI 或第 2 阶段首次应用，不追溯本轮）：

1. 聚合判据读 `acceptance_status.json`；被裁定为**参照无效**（非"参照不可达"）的模型从聚合中剔除，并在输出里点名剔除了谁、依据哪条裁定
2. 同时报告含与不含这些模型的两套结果，像现有的 ef_lstm 敏感性行那样
3. **"参照不可达"不构成剔除理由**——那是被检验的主张本身，剔除它等于回避检验

**当前应当如实报告的表述**：对能复现的参照，MAE 打平（Z=+0.40, p=0.69）；Corr 系统性落后（Z=+3.35），**但该结论由两个参照值已被裁定的模型主导**——mctn 的参照数值无定义，ef_lstm 的差距来自 23% 崩溃率下的抽样运气。去掉这两个的敏感性分析尚未由判据自动给出，需人工计算，本轮不据此改判。

## <a id="dlf-task-heads"></a>DLF 的全面缺口：漏了四项任务损失，而等价测试看不见（2026-08-03）

我们的 DLF 对**作者代码**参照在**六个指标上同时落后**：MAE +3.7 SE、Corr −3.0 SE、Acc-7 −4.0 SE、Acc-2(non0) −1.8 SE、F1 与 Acc-5 同向。而权重复制的等价测试给的是 **8.941e-08**。

**模型数值等价 + 六个指标同号偏差 = 协议差异，不是种子噪声。** 这是回去翻 trainer 的唯一理由，也是本条的全部价值。

### 现象与归因

`trains/singleTask/DLF.py:81-89` 的任务损失是**五个头**，不是一个：

```python
loss_task = 1 * loss_task_all + 1 * loss_task_c \
          + 3 * loss_task_l_hetero + 1 * loss_task_v_hetero + 1 * loss_task_a_hetero
```

我们的 `compute_loss` 只算了 `loss_task_all`（融合后的最终预测）。**漏掉的四项里有三项的头根本不进最终预测，只被监督。**

那个 **3** 是「语言聚焦」的另一半：

| 半 | 在哪里 | 论文写了吗 |
|---|---|---|
| 结构 | 只有语言当 query；`trans_a_with_l` 等四条通路构造了但不接线 | 否 |
| 目标 | 语言头权重 3，其余 1 | 否 |

**论文两处都没写。**只读论文重写这个模型，两处都会漏。

### 为什么等价测试没抓住（本条最该被记住的部分）

测试原来的理由是我自己写进 docstring 的：

> 最终预测对上了，喂给它的每一级就都对上了，中间量隐式被检查。

这句话**没错，但对四个辅助头里的三个完全不适用** —— 它们不在最终预测的上游。

> **「上游自动被覆盖」只覆盖你所比较的那个张量的上游。只为被监督而存在的头，必须点名比较。**

修正后测试点名比较全部五个头，一致到 **2.384e-07**。**结构本来就是对的**，错的只有损失 —— 这也说明等价测试的诊断力取决于比较面选得对不对，而不取决于它是否通过。

### 判据的第二个盲区：同向偏差

出问题的那一轮，`check_acceptance.py` 给的判定是 **reproduced**。NOISE_FLOOR 把每个指标单独吸收掉了，而**判据里没有任何一条会看「六个偏差全部同向」**。

与 `#rebaseline-5080` 记的那个盲区（聚合判据不读裁定）性质相同：判据逐项看是合理的，合起来漏掉了一个信号。

**本轮不改判据**——同 `#rebaseline-5080` 的处理：改动在已经看见它会带来什么之后再做，就不是预注册而是事后修饰。**预注册于此，留到 MOSEI/Phase 2 一并评估**：单模型判据应增加一个"同向偏差"维度，例如 k 个指标全部同号且平均幅度超过 1 SE 时降级为 flagged。

### 连带修掉：这个测试当时是静默失效的

`check_dlf_equivalence.py` 没有 `MMSA_SHIM` 就因 `pynvml` 缺失而 SKIP，**而 SKIP 退出码是 0，`check_all.sh` 记成 PASS**。与 `#almt-silent-skip` 同一个失效形态。现在它自己解析 shim 路径。

**同一个坑第三次出现（ALMT、这次、以及 DPDF-LQ 当时靠手动导出躲过）。规矩：任何以 SKIP 兜底的闸门，必须自己解析依赖路径，不得依赖调用者导出环境变量。**
...
## <a id="intensity-weighting"></a>按情感强度加权，到底有没有用？（预注册于 2026-08-05，**跑之前**）

### 为什么这个问题可以回答，而不只是可以争论

两篇论文提出同一个想法——**对比学习的正负样本对按标签距离加权**——而两边的开关都在代码里现成：

| | 论文声称 | 代码实际 | 开关 |
|---|---|---|---|
| **ConFEDE**（ACL 2023） | 负样本按标签距离加权 | **从未启用**：`cont_NTXentLoss` 只有调用 `update_label` 才加权，三份定义无一处调用 | 把它接上 |
| **CLGSI**（NAACL 2024） | 正负对按情感强度加权 | 启用 | 掩码那行下面**紧挨着注释掉的 `= 1`**（不加权），作者试过、论文没报 |

两个实现都已通过权重复制等价测试（ConFEDE 前向 `0.000e+00`；CLGSI 前向 `0.000e+00`、对比损失 `1.863e-08`），所以**开关之外的一切都是受控的**。

### 预注册的预测（写在跑之前，不得事后修改）

**我预测这两个改动的效应都小于种子噪声，即 |Δ| < 1 SE，两个方向都不显著。**

依据：本项目已测得 MOSI 上模型间真实差距普遍在 0.01–0.03 MAE，而单模型种子标准差是 0.01–0.04；一个只改变损失中样本对权重的改动，很难越过这个量级。**若预测错了——尤其若关掉加权反而更好——那比预测对了更有价值。**

### 设计

四个组，各 10 seed（42-51），只改开关：

| 组 | 相对基线 |
|---|---|
| `clgsi_mosi_ablation_unweighted` | 掩码改为二值（作者注释掉的那版） |
| `confede_mosi_ablation_weighted` | 接上 `update_label`，启用负样本加权 |

与已有的 `clgsi_mosi`、`confede_mosi` 配对比较，报**配对差**与 SE。

### 判读规则（同样预注册）

- |Δ| < 1 SE：**该机制在 MOSI 上不可检出**
- 1–2 SE：方向可疑，不足以断言
- \> 2 SE 且两篇同向：**该机制真实有效**，且论文没报的那半（ConFEDE 从未启用）是实现缺陷
- \> 2 SE 但两篇反向：机制依赖具体架构，不能作为一类方法的共同结论

**不看结果再定规则**——这条与判据类改动的处理一致（见 `#rebaseline-5080`）。

### 结果（2026-08-05）

| 组 | MAE | Δ（SE） | Corr | Δ（SE） |
|---|---|---|---|---|
| CLGSI 加权（基线） | 0.7498 ± 0.0429 | — | 0.7822 ± 0.0103 | — |
| CLGSI **不加权** | 0.7545 ± 0.0316 | +0.0047（**+0.3**） | 0.7826 ± 0.0073 | +0.0004（+0.1） |
| ConFEDE 不加权（基线，即 release 的实际行为） | 0.7358 ± 0.0185 | — | 0.7872 ± 0.0078 | — |
| ConFEDE **加权**（接上从未启用的开关） | 0.7341 ± 0.0192 | −0.0017（**−0.2**） | 0.7873 ± 0.0081 | +0.0001（0.0） |

**四个配对差全部 |Δ| < 1 SE。按预注册规则：该机制在 MOSI 上不可检出。**

而且两篇的方向还相反——CLGSI 关掉加权 MAE 略差，ConFEDE 打开加权 MAE 略好——**但两个方向都在噪声里，不构成"方向不一致"的证据，只是噪声该有的样子**。

### 这个实验能说什么、不能说什么

**能说**：在 MOSI、10 seed、两套已验证等价的实现上，**按情感强度给对比样本对加权的效应小于本设计的检出下限**。

**不能说**"该机制无效"。本设计的检出下限是 **2 SE**：CLGSI 上 0.0337 MAE、ConFEDE 上 0.0169 MAE。小于这个量级的真实效应，本实验看不见。**而这两个下限本身就已经宽于文献里大多数"改进"所报告的幅度**——这才是更值得说的一句。

### 预注册的预测被证实了，但真正的收获在别处

预测（|Δ| < 1 SE，两个方向都不显著）与结果一致。**预测对了本身没什么价值**——有价值的是这件事：

**ConFEDE 把「按标签距离加权」写进论文当贡献，代码里却从未启用。我们把它接上，跑 10 seed，MAE 变化 0.0017（0.2 SE）。也就是说，这个从未生效的贡献，即使生效也测不出来。**

同样地，CLGSI 把它启用并作为方法核心，而关掉它的代价是 0.3 SE。

### 途中修掉的一个陷阱

第一次跑出的 Δ 是**精确的 0.0000，连标准差都相同**——因为 `--model-arg x=false` 被解析成字符串 `"false"`（真值），开关根本关不掉。**若差值是个看着合理的小数，我会把它当作"效应不可检出"写进结论，而那正是我预注册的预测。** 已修解析器；既有各组均用大写字面量，未受影响。

> **消融组与对照组结果完全相等，不是零效应，是开关坏了。真正的零效应里是有噪声的。**
