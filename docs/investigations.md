...
> **2026-09-10 当日更正：方向我推反了。** 这里原本写"它抬高的正是论文要报的那些
> 数字"、"方向必然是让论文数字更好"。跑了对照臂，**clean 臂逐 seed 更好**（seed 42
> MAE 0.7020 对 0.7127，seed 43 0.7431 对 0.7484）。重新推导：
>
...
>
> 所以正确的表述是：**泄漏是真的（测试集确实被碰了），但它压低而不是抬高了 HSCL 的
> 发表数字。** 逐轮数字见 `scripts/hscl_selection_trace.py`。
>
> 这条记下来的价值不在结论本身，而在于**我有一个听起来很紧的机制推断，方向是反的，
> 两个 seed 的对照臂就把它推翻了**。机制推断不能替代对照。
